...
class RendaFixa:
    ...
    def insert(self):
        ...
        try:
            with self.conexao.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO rendafixa (datalancamento, valor, descricao, idtitulorendafixa, idconta) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                    (self.data_lancamento, self.valor, self.descricao, self.id_titulo_renda_fixa, self.id_conta),
                )
    ...
    def update(self):
        ...
        try:
            with self.conexao.cursor() as cursor:
                cursor.execute(
                    "UPDATE rendafixa set datalancamento = %s, valor = %s, descricao = %s, idtitulorendafixa = %s, idconta = %s where id = %s;",
                    (self.data_lancamento, self.valor, self.descricao, self.id_titulo_renda_fixa, self.id_conta, self.id),
                )
                self.conexao.commit()
                self.conexao.close()
        except Exception as e:
    ...
    def select_by_id(self, id):
        ...
        retorno = False
        with self.conexao.cursor() as cursor:
            cursor.execute("SELECT id, datalancamento, valor, descricao, idtitulorendafixa, idconta FROM rendafixa WHERE id = %s", (id,))
            row = cursor.fetchone()
            if row:
                self.id = row[0]
    ...
    def get_all(self):
        ...
        self.conexao = RendaFixa.getConexao()
        with self.conexao.cursor() as cursor:
            cursor.execute("SELECT rf.id, rf.datalancamento, rf.valor, rf.descricao, rf.idtitulorendafixa, rf.idconta, "
                           "trf.nometitulo, c.nomeconta FROM rendafixa as rf "                       
